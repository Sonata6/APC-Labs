...
void print()
{
	char temp;  // temp bit
	int i, val;
	struct VIDEO far* screen = (struct VIDEO far *)MK_FP(0xB800, 0);   // get Video card buffer                 
																												 
	val = inp(0x21);                                     // get mask Master registor
	for (i = 0; i < 8; i++)                              // find bits
	{
		temp = val %2;                                   // get last bit
		val = val >> 1;                                  // move bits on right												//inp - вывод из порта в переменную, outp ввести в порт из переменной.
...
void interrupt(*oldint10) (...);// IRQ 2  - interrupt for cascade interruptions in AT machines (для связи со слейвом, с которым мастер соединен каскадно).
...
void initialize()
{
	...
	setvect(0x8, newint08);  //set new handler for IRQ 0
	setvect(0x9, newint09);  //set new handler for IRQ 1
	setvect(0xA, newint0A);  //set new handler for IRQ 2
	setvect(0xB, newint0B);  //set new handler for IRQ 3
	setvect(0xC, newint0C);  //set new handler for IRQ 4
	setvect(0xD, newint0D);  //set new handler for IRQ 5
	setvect(0xE, newint0E);  //set new handler for IRQ 6
	setvect(0xF, newint0F);  //set new handler for IRQ 7

	setvect(0x80, newintC0);  //set new handlers for IRQ 8
	setvect(0x81, newintC1);  //set new handlers for IRQ 9
	setvect(0x82, newintC2);  //set new handlers for IRQ 10
	setvect(0x83, newintC3);  //set new handlers for IRQ 11
	setvect(0x84, newintC4);  //set new handlers for IRQ 12
	setvect(0x85, newintC5);  //set new handlers for IRQ 13
	setvect(0x86, newintC6);  //set new handlers for IRQ 14
	setvect(0x87, newintC7);  //set new handlers for IRQ 15
	_disable(); // CLI
				// interrupt initializtion for Master
	outp(0x20, 0x11);	//ICW1 - initialize master
	outp(0x21, 0x8);   //ICW2 - base vector for master
	outp(0x21, 0x04);	//ICW3 - the port bit of Slave (in binary format)        00000000 00000100 slave connected
